...
// it with the class label given by the buttons of the appliances are
...
async function predict() {
  ...
  while (isPredicting) {
    ...
    //This logic is for fixed number of gestures (4) - one for start, one for stop, two for items. Need to work on it to make it generic
      switch(classId){

        case 0://if(prevId==-1){flag1 =0;}
           pred = [];
           prevId=0;
           break;
        case 1:if(prevId==0 || prevId==3)
          {
            if(prevId == 0&& flag1==0){
...
function sendata(prediction){
  ...
  var request = new XMLHttpRequest();
    ...
    request.open('POST', 'http://localhost:3000/pred', true);
    request.setRequestHeader("Content-Type", "application/json");
    var data = JSON.stringify(prediction);
    request.send(data);
